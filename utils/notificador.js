...
exports.verificarNotificacoes = () => {
  ...
  db.query(sql, (err, acoes) => {
    ...
    acoes.forEach(acao => {
      ...
      if (diasRestantes < 0) {
        criarNotificacao(acao.responsavel_id, acao.id, `⚠️ Ação "${acao.titulo}" está atrasada!`);
          ...
          (err, coordenadores) => {
            if (!err) {
              coordenadores.forEach(coord => {
                criarNotificacao(coord.id, acao.id, `⚠️ Ação "${acao.titulo}" atribuída a ${acao.responsavel_id} está atrasada.`);
              });
            }
          }
        );
