...
exports.login = (req, res) => {
  ...
  db.query('SELECT * FROM usuarios WHERE email = ?', [email], async (err, results) => {
    ...
    res.json({ token, usuario: { id: user.id, nome: user.nome, funcao: user.funcao } });
  });
};
...
exports.cadastrar = async (req, res) => {
  ...
  db.query('INSERT INTO usuarios (nome, email, senha, funcao) VALUES (?, ?, ?, ?)',
    [nome, email, hash, funcao], (err, results) => {
      if (err) return res.status(500).json(err);
